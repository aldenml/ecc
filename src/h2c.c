...
void ecc_h2c_expand_message_xmd_sha256(
    ...
    const int b_in_bytes = 32;
    const int r_in_bytes = 64;
    ...
    byte_t Z_pad[64] = {0}; // r_in_bytes = 64
    ...
    crypto_hash_sha256_update(&st, l_i_b_str, 2);
    // I2OSP(0, 1)
    byte_t tmp[1] = {0};
    ecc_I2OSP(tmp, 0, 1);
    ...
    // b_0 = H(msg_prime)
    byte_t b_0[32];
    crypto_hash_sha256_final(&st, b_0);

    // 8.  b_1 = H(b_0 || I2OSP(1, 1) || DST_prime)
    crypto_hash_sha256_init(&st);
    // b_0
    crypto_hash_sha256_update(&st, b_0, 32);
    // I2OSP(1, 1)
    ecc_I2OSP(tmp, 1, 1);
    crypto_hash_sha256_update(&st, tmp, 1);
    // DST_prime
    ...
    byte_t b_1[32];
    crypto_hash_sha256_final(&st, b_1);
    ...
    for (int i = 2; i <= ell; i++) {
        byte_t *b_prev = &uniform_bytes[(i - 2) * 32]; // b_(i - 1)
        byte_t *b_curr = &uniform_bytes[(i - 1) * 32]; // b_i
        crypto_hash_sha256_init(&st);
        // strxor(b_0, b_(i - 1))
        byte_t bxor[32];
        ecc_strxor(bxor, b_0, b_prev, 32);
        crypto_hash_sha256_update(&st, bxor, 32);
        // I2OSP(i, 1)
        ecc_I2OSP(tmp, i, 1);
        crypto_hash_sha256_update(&st, tmp, 1);
        // DST_prime
        ...
        crypto_hash_sha256_final(&st, b_curr);
    }
