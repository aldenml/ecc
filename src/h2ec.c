...
void ecc_h2ec_expand_message_xmd_sha512(
    ...
    crypto_hash_sha512_state h_state;
    ...
    // 6.  msg_prime = Z_pad || msg || l_i_b_str || I2OSP(0, 1) || DST_prime
    // 7.  b_0 = H(msg_prime)
    crypto_hash_sha512_init(&h_state);
    byte_t Z_pad[128] = {0}; // r_in_bytes = 128
    // Z_pad = I2OSP(0, r_in_bytes)
    crypto_hash_sha512_update(&h_state, Z_pad, r_in_bytes);
    // msg
    crypto_hash_sha512_update(&h_state, msg, msg_len);
    // l_i_b_str = I2OSP(len_in_bytes, 2)
    byte_t l_i_b_str[2] = {0};
    ecc_I2OSP(l_i_b_str, len_in_bytes, 2);
    crypto_hash_sha512_update(&h_state, l_i_b_str, 2);
    // I2OSP(0, 1)
    byte_t tmp[1] = {0};
    ecc_I2OSP(tmp, 0, 1);
    ...
    // DST_prime
    //  - DST
    crypto_hash_sha512_update(&h_state, dst, dst_len);
    //  - I2OSP(len(DST), 1)
    ...
    crypto_hash_sha512_init(&h_state);
    // b_0
    crypto_hash_sha512_update(&h_state, b_0, 64);
    // I2OSP(1, 1)
    ecc_I2OSP(tmp, 1, 1);
    crypto_hash_sha512_update(&h_state, tmp, 1);
    // DST_prime
    ...
    for (int i = 2; i <= ell; i++) {
        ...
        crypto_hash_sha512_update(&h_state, bxor, 64);
        // I2OSP(i, 1)
        ecc_I2OSP(tmp, i, 1);
        crypto_hash_sha512_update(&h_state, tmp, 1);
        // DST_prime
        ...
        crypto_hash_sha512_final(&h_state, b_curr);
    }
